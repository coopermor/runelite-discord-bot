...
import { filteredWordsDb, roleDb, noteDb } from './db.js'
...
function isChannelProtected (channel) {
  return channel && channel.guild && config.channels.notesChannels.includes(channel.name)
}
...
function buildUserDetail (user, withMention = true, withNote = false) {
...
function processFilters (message, client, isEdit = false) {
  try {
    ...
    const dmMessageDetail = filteredResult.dmReason
      ? `message was filtered.\n**Reason:** ${filteredResult.dmReason}.` + buildMessageDetail(message)
      : messageDetail
...
function messageContentFilter (messageContent) {
  return filteredWordsDb.ls().find(w => messageContent.match(new RegExp(w, 'gi')))
}
...
function messageFilter (message, client, isEdit) {
  ...
  if (isEdit && Date.now() - message.createdTimestamp > config.spam.editFilterMaxAge) {
    return false
  }
  ...
  const currentMessage = {
    ...
    content: remove(message.cleanContent.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')),
    attachmentsHash: hashAttachments(message.attachments.map(a => a)),
  ...
  const duplicateMatches = cachedMessages.filter((m) => (m.content === currentMessage.content && m.attachmentsHash === currentMessage.attachmentsHash) && (m.sentTimestamp > (currentMessage.sentTimestamp - config.spam.maxDuplicatesInterval)))
...
function hashAttachments (attachments) {
  ...
  for (const attachment of attachments) {
    hash += attachment.size + attachment.height + attachment.width
  }
  return hash
}
